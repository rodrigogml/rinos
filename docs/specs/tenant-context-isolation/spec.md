...
- Q: Contrato pessoal pode acompanhar a troca de tenant? -> A: Não. Operações tenant avaliam somente o contrato do tenant explícito; operações pessoais não usam tenant ativo, e contexto global administrativo não infere contrato pessoal.
...
**Independent Test**: selecionar a conta A, trocar para a conta B e verificar que a nova área contém somente dados
e acesso efetivo após permissões e bloqueios vigentes da conta B, preservando A quando a troca falhar.

**Acceptance Scenarios**:

1. **Given** contexto válido da conta A, **When** o usuário troca com sucesso para a conta B, **Then** identidade,
   associação, estado, permissões, bloqueios e funcionalidades aplicáveis são reavaliados antes da substituição
...
4. **Given** revogação de permissão ou novo bloqueio sem perda da associação, **When** o usuário executa nova
   operação protegida, **Then** a decisão resolve as regras vigentes e não uma autorização antiga da seleção
...
- Uma tarefa assíncrona começa depois que o usuário originador perdeu a permissão ou recebeu bloqueio para a chave.
...
- **FR-TCI-SEL-009**: Conta suspensa DEVE admitir somente contexto restrito às ações de consulta ou regularização
  com permissão vigente, sem bloqueio vigente e admitidas pelas regras de estado.
...
- **FR-TCI-WRK-002A**: Na superfície Vaadin, a referência mínima da conta, tenant e associação selecionados DEVE
  pertencer à `UI`/área de trabalho exata; `HttpSession`, `VaadinSession`, `RFWSessionState`, `SecurityContext` e
  principal NÃO DEVEM manter um único tenant ativo nem chaves ou decisões efetivas compartilhadas.
...
- **FR-TCI-SWITCH-002**: Falha na troca NÃO DEVE deixar identificadores, regras, decisões, funcionalidades ou dados
  transitórios do novo tenant no contexto anterior.
...
- **FR-TCI-VALID-002**: A seleção da conta NÃO DEVE congelar autorização; cada operação protegida DEVE
  reavaliar permissões e bloqueios vigentes definidos em `access-control`.
- **FR-TCI-VALID-002A**: Capacidades consultadas para compor menus e ações são projeções de interface e NÃO DEVEM ser
  usadas como autoridade pela facade ou serviço; a tentativa direta sempre produz nova decisão canônica.
...
- **FR-TCI-VALID-008**: Operação cuja execução já tenha sido aceita após validação do contexto e da autorização
  DEVE poder terminar no tenant original mesmo que sua permissão, ausência de bloqueio, associação ou conta seja
  invalidada durante o processamento; repetição ou novo efeito exige outra decisão.
...
- **FR-TCI-PROP-008**: `RFWExecutionContext` PODE transportar uma projeção pequena e imutável do contexto delimitado
  pela entrada, mas NÃO DEVE ser usado como armazenamento de sessão, cache ACL ou substituto da validação de domínio.
...
- **FR-TCI-BOUND-003**: Grupos, chaves, regras de permissão ou bloqueio e explicação de acesso pertencem a
  `access-control`; esta feature não transforma seleção em autorização.
...
- **FR-TCI-BOUND-007**: Seleção, troca ou invalidação de tenant NÃO DEVE combinar contrato pessoal, contrato de outro tenant ou consumo de outro titular.
...
- **FR-TCI-INFRA-CACHE-001**: Cache de autorização DEVE ser particionado ao menos por tenant e associação, confirmar a
  revisão autoritativa por nova operação e respeitar fronteiras temporais mesmo quando não houver mutação.
...
- **SC-TCI-011**: Em 100% dos testes com duas `UI`s na mesma sessão, cada uma conserva seu tenant e associação sem
  compartilhar chaves, decisões ou seleção com a outra.

## Integração com Controle de Acesso

Cada entrada protegida reavalia as regras e bloqueios vigentes no tenant explícito. Operação já iniciada segue a política da feature originadora, mas repetição ou trabalho ainda não iniciado exige nova decisão canônica.
