...
**Actions and Behavior**: Validar localmente presença, formato, confirmação da senha e aceites obrigatórios; submeter pelo `RFWRegistrationProvider`; aceitar colagem e gerenciadores; abrir documentos em nova aba sem perder o preenchimento; impedir submissões repetidas enquanto processa; preservar e-mail e aceites após rejeição recuperável, mas sempre limpar senha, confirmação e token Turnstile. A submissão registra as versões publicadas efetivamente apresentadas; eventual substituição posterior não reabre este formulário e será resolvida obrigatoriamente na ativação.
...
| partial-stale | N/A — mudança posterior das versões não invalida a criação da pendência; a ativação compara e exige os documentos obrigatórios vigentes | N/A | N/A |
...
**Entry and Navigation**: Aberta após cadastro local aceito ou pelo deep link HTTPS canônico `/login?step=activation&proof=...`. A URL não contém e-mail nem ID interno. Depois que a rota entrega a prova ao estado efêmero do `RFWAccessComponent`, substitui a entrada visível do histórico por `/login`; atualizar ou voltar não reapresenta a prova. A entrada `/login?step=activation` abre o mesmo formulário para colagem manual. Ação Voltar retorna ao login. Ativação ordinária publica a autenticação e navega ao Painel de Usuário; a identidade fundadora definida pela política segura navega somente ao enrollment TOTP obrigatório.
**Content and Data**: Explicação de validade; identificador apenas quando necessário para código manual ou reenvio; a mesma prova opaca de 256 bits recebida no link e exibida no e-mail como código copiável; destino mascarado quando seguro; expiração; ação Ativar; Reenviar confirmação; Cancelar cadastro; Voltar para entrar. Não existe código curto ou segunda credencial. Se os documentos obrigatórios mudaram, uma continuação separada mostra somente as novas versões antes da ativação. Nessa continuação, o e-mail comprovado é somente leitura e preserva apenas o primeiro caractere do local seguido de `***` e do domínio completo, como `p***@example.com`; locais de até dois caracteres são integralmente ocultados como `***@example.com`.
**Actions and Behavior**: Consumir a prova uma única vez; aceitar link ou código; cadastro aceito e reenvio aceito permanecem em `ACTIVATION`, com confirmação pública, em vez de encerrar a jornada em `RESULT`; reenvio invalida provas anteriores e respeita três solicitações por quinze minutos; repetição de prova já concluída não repete efeitos; novo aceite legal usa referência opaca própria; cancelamento abre INT-WEB-REG-004.
**Validation and Feedback**: Quando a política segura exigir TOTP, informar somente que a confirmação adicional é obrigatória, sem revelar a origem da política nem o e-mail configurado; não oferecer tenant, grupo ou privilégio nessa jornada. Diferenciar instrução ausente, inválida, expirada, usada e cadastro encerrado sem expor dados adicionais. Prova usada não autentica novamente nem repete a ativação e orienta o retorno ao acesso; cadastro cancelado, marcado como expirado ou além da validade absoluta informa que uma nova inscrição é necessária. Oferecer reenvio quando permitido e informar o tempo restante de limite. Falha de SMTP mantém a pendência, não afirma entrega e permite nova tentativa explícita; mudança legal impede ativação até aceite das versões vigentes.
...
| success | Identidade ativa; confirmação breve antes da navegação segura | Continuar ao Painel de Usuário ou ao enrollment TOTP obrigatório, conforme política segura | Painel de Usuário ou enrollment TOTP |
...
**Entry and Navigation**: O Google é iniciado na etapa de login do `RFWAccessComponent`. Resultado `EXTERNAL_REGISTRATION_REQUIRED` abre automaticamente `EXTERNAL_REGISTRATION` na mesma rota. Voltar descarta a continuação visual e retorna ao login. Conclusão ordinária publica a autenticação e navega para a rota global autenticada `/user`, reservada ao Painel de Usuário; quando a política segura exigir TOTP, navega apenas ao enrollment obrigatório. Seu conteúdo e suas operações pertencem a `user-dashboard`.
...
**Validation and Feedback**: E-mail não pode ser editado; aceites inválidos focam o primeiro documento; a exigência de TOTP posterior não revela a política ou o e-mail configurado; referência expirada, usada, identidade conflitante ou Google inválido não cria usuário; indisponibilidade oferece tentar Google novamente ou usar cadastro local; nunca revelar outro usuário relacionado ao e-mail ou à identidade externa.
...
| success | Identidade ativa pelo Google e autenticação publicada | Continuar ao Painel de Usuário ou enrollment TOTP obrigatório | `/user` ou enrollment TOTP |
