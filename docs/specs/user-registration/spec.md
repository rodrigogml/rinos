...
- **FR-REG-005**: O sistema DEVE exigir aceite explícito das versões publicadas dos termos de uso e da política de privacidade apresentadas no formulário antes de criar o cadastro pendente. Se uma dessas versões deixar de ser vigente depois de apresentada e antes da submissão, o sistema DEVE registrar exatamente a versão aceita e PODE criar a pendência, mas NÃO DEVE considerar esse aceite suficiente para a ativação.
...
- **FR-REG-016**: A ativação DEVE comparar os aceites imutáveis do usuário com as versões então vigentes dos documentos obrigatórios e exigir todas as versões ausentes, além de uma das seguintes comprovações: confirmação válida do e-mail no cadastro local ou identidade Google validada com e-mail verificado.
...
- **FR-REG-047**: Quando existir cadastro pendente com o mesmo e-mail normalizado, o fluxo Google DEVE reutilizar essa identidade e PODE criar ou substituir uma única identidade externa candidata no estado `PENDING` para sustentar a continuação dos aceites. Na conclusão, o sistema DEVE invalidar a senha local, as comprovações e os dados temporários anteriores e registrar os novos aceites antes de ativar a candidata externa, o cadastro e o usuário.
