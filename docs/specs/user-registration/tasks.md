...
- [x] 1.5.8 Criar testes com duas instâncias, mesmo `instanceId`, reinício, expiração, retorno da líder antiga, lote antigo bloqueado, timeout, ausência de sobreposição, relógio local divergente e banco indisponível
...
- [x] 6.2.8 Criar testes de componente, E2E de retomada e inspeção visual nos form factors
  - Evidência: [testes e inspeção visual da retomada em desktop e telefone](./evidence/6.2.8/README.md).
...
- [x] 6.3.7 Criar testes de componente, integração Google simulada, E2E e inspeção visual
  - Evidência: [jornada Google simulada, sessão, E2E e inspeção visual](./evidence/6.3.7/README.md).
...
- [x] 6.5.4 Cobrir prova inválida, expirada, usada e processo encerrado
...
- [x] 6.5.7 Criar testes de componente, E2E destrutivo e inspeção visual registrada
  - Evidência: [componente, E2E, MySQL destrutivo e inspeção visual](./evidence/6.5.7/README.md).
...
- [x] 7.1.7 Executar build e testes do Rinos e validação isolada do RFW referenciado
  - Evidência: [build limpo encadeado do RFW, showroom e Rinos](./evidence/7.1.7/README.md).
- [x] 7.1.8 Atualizar `tasks.md`, documentação e evidências com os resultados
  - Evidência: [consolidação da matriz integrada e dos gates remanescentes](./evidence/7.1.8/README.md).
...
- [ ] 7.4.2 Confirmar ao menos 95 aceitações em até dois minutos, restaurar e validar separadamente o limite padrão de 20 e o Turnstile real e executar um único smoke test no SMTP real sem declarar throughput `[release-blocker]`
  - Evidência atualizada: [SLO local, limite 20, Siteverify externo e SMTP real validados; demais critérios do gate permanecem pendentes](./evidence/7.4.2/README.md).
...
- [x] 7.5.6 Executar a calibração Argon2id no perfil de servidor-alvo e registrar hardware, JVM, parâmetros, data, mediana e percentil 95 `[release-blocker]`
  - Evidência: [calibração efetiva no Turing, aplicação de 75 iterações e smoke test pós-reinício](./evidence/7.5.6/README.md).
...
- [x] 8.4 Executar backfill dos usuários ativos antes de habilitar a invariável obrigatória.
- [x] 8.5 Atualizar testes, checklist e evidências sem apagar a evidência histórica de ausência de tenant.
- [x] 8.6 Integrar a política exclusiva do fundador ao fluxo de ativação, exigindo confirmação TOTP antes do Painel de Usuário e sem criar tenant, grupo ou privilégio.
  - Evidência: [continuação de ativação e restrição exclusiva ao enrollment TOTP](./evidence/8.6/README.md).
- [x] 8.7 Validar cadastro local e Google do fundador, falha/abandono de enrollment, sigilo do e-mail configurado e impossibilidade de acessar jornadas autenticadas antes do TOTP.
  - Evidência: [matriz dos cenários do fundador e preservação da restrição](./evidence/8.7/README.md).
