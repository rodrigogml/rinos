...
Esta feature estabelece o fato monetário interno que explica alterações no saldo de uma conta financeira. Um lançamento confirmado representa uma entrada ou saída reconhecida pelo tenant, possui valor na moeda da conta, data de efetivação, classificação econômica, dimensões aplicáveis, origem e histórico completo de suas revisões.

O lançamento pode ser criado manualmente ou originado por outro módulo. Rascunhos ajudam a preparar e revisar dados, mas não alteram saldos. Enquanto sua data estiver aberta pelo fechamento financeiro do tenant, o lançamento confirmado pode ser revisado ou cancelado logicamente com histórico integral e recálculo dos saldos afetados. Depois de bloqueado, ele se torna imutável e correções produzem novos lançamentos compensatórios vinculados.
...
- **FR-FTR-CORE-008**: Lançamento confirmado com data futura DEVE permanecer programado, compor projeções aplicáveis e NÃO DEVE afetar saldo atual nem ser apresentado como fato já efetivado antes da data.
...
- **FR-FTR-POST-003**: Confirmação bem-sucedida DEVE produzir um único lançamento; seu efeito DEVE ser visível no saldo somente quando a data de efetivação estiver incluída na referência consultada.
...
- **FR-FTR-POST-008**: Confirmação, revisão ou cancelamento retroativo autorizado DEVE recalcular saldos derivados posteriores sem modificar identidades nem históricos de outros lançamentos.
...
- **FR-FTR-DATE-008**: Saldos derivados por lançamento, dia e intervalo DEVEM ser reproduzíveis a partir da abertura e do conteúdo vigente dos lançamentos, enquanto seus históricos explicam todas as revisões, cancelamentos e inclusões retroativas permitidas.
- **FR-FTR-DATE-009**: Quando houver data de fechamento, data de efetivação igual ou anterior a ela NÃO DEVE aceitar novo lançamento confirmado, revisão, cancelamento, estorno, correção nem efeito solicitado por módulo.
...
- **FR-FTR-SEARCH-004**: Rascunhos, programados, efetivados, revisados, cancelados, estornos e lançamentos estornados DEVEM ser visualmente distinguíveis sem ocultar o efeito vigente.
...
- **FR-FTR-SEC-005**: Criação, alteração e exclusão de rascunho; confirmação, revisão e cancelamento de lançamento, tentativa repetida divergente, estorno, correção, lançamento retroativo, negação sensível e exportação DEVEM registrar ator, tenant, instante, origem, motivo, resultado e efeito.
...
- **FR-FTR-INFRA-LOCK**: Confirmações, revisões, cancelamentos, estornos, correções e inclusões retroativas concorrentes DEVEM preservar unicidade, histórico, ordenação e saldo determinístico em todas as instâncias da aplicação.
